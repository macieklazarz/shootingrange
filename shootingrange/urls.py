...
from mainapp.views import (home_screen_view, zarzadzanie, BlogMainView, Payment, payment_redirect, post_payment,  BlogUserView, BlogUserUpdateAdminView, PostDetailView, PostCreateView, ForumListView, ForumTopicCreateView, ForumPostCreateView, ForumPostDetailView, blog_registration)
...
from wyniki.views import (wyniki_edycja, wyniki_dynamic_edycja,  wyniki, wyniki_general, wyniki_druzynowe, exportexcel, exportexceldynamic, exportexcelgeneralka, export_psv_wyniki_general,  WynikUpdateView, WynikDynamicUpdateView, not_authorized, RejestracjaNaZawodyView,  RejestracjaNaZawodyDynamicView, KonkurencjaDeleteView, KonkurencjaDynamicDeleteView, TurniejListView, TurniejDeleteView, TurniejEditView, TurniejCreateView, OplataListView, OplataDynamicListView, OplataUpdateView, OplataDynamicUpdateView, UczestnikDeleteView, BronAmunicjaListView, rejestracja_choose)
from django.conf import settings
from django.conf.urls.static import static

urlpatterns = [
    path('admin/', admin.site.urls),
    # path('json/', view_accounts, name='view_accounts'),
    # path('start/', StronaStartowaListView.as_view(), name="start"),
    path('start/', StronaStartowaListView.as_view(), name="start"),
    path('', BlogMainView.as_view(), name="BlogMainView"),
    path('forum/', ForumListView.as_view(), name="ForumListView"),
    path('blog_users/', BlogUserView.as_view(), name="BlogUserView"),
    # path('payment/', Payment.as_view(), name="Payment"),
    path('payment/', Payment.as_view(), name="Payment"),
    path('payment_redirect/', payment_redirect, name="payment_redirect"),
    path('post_payment/', post_payment, name="post_payment"),
    # path('secure.przelewy24.pl/trnRequest/<TOKEN:str>/', PaymentRedirect.as_view(), name="payment_redirect"),
    path('forum_topic_create/', ForumTopicCreateView.as_view(), name="ForumTopicCreateView"),
    path('forum_post_create/', ForumPostCreateView.as_view(), name="ForumPostCreateView"),
    path('forum_post_detail/<slug:slug>/', ForumPostDetailView.as_view(), name="ForumPostDetailView"),
    path('blog_user_update/<int:pk>/', BlogUserUpdateAdminView.as_view(), name="BlogUserUpdateAdminView"),
    path('login/', auth_views.LoginView.as_view(template_name='mainapp/blog_login.html'), name="LoginView"),
    path('logout/', auth_views.LogoutView.as_view(template_name='mainapp/blog_logout.html'), name="LogoutView"),
    path('post/<slug:slug>/', PostDetailView.as_view(), name="PostDetailView"),
    path('post_create/', PostCreateView.as_view(), name="PostCreateView"),
    path('blog_registration/', blog_registration, name="blog_registration"),
